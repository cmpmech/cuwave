...
# projection
SCHEME = "exponential"
BETA, BETA_MAX, STAGES = 1.0, 8.0, 4
...
chain = lambda x, gradient: torch.as_tensor(
    projection.grad(x, (1.0 - GAMMA_MIN) * gradient)
)[None, None]
...
for iteration in range(ITERS):
    ...
    optimizer.zero_grad()
    field.backward(chain(x, gradient))
    optimizer.step()
...
# final misfit
with torch.no_grad():
    gamma = indicator(to_cupy(generator()))
