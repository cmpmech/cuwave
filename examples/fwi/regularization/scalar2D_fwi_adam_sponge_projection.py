...
SCHEME = "staircase"  # BETA == BETA_MAX here, so every scheme is the same ramp
...
damping = sponge(
    sim, cp.ones(sim.Nx_padded, dtype=sim.dtype), pad, SPONGE_BETA, faces=SPONGE_FACES
)
sim = replace(sim, damping=damping)
sim_obs = replace(scalar_wave(N_obs, dt_obs, SPACE_ORDER_OBS), damping=damping)
print(
    f"sponge {pad} nodes per side at beta {SPONGE_BETA}, "
    f"{Nx[0]} x {Nx[1]} nodes in total"
)
