...
from cuwave.geometry import circles, stacked_circles
...
MASK_RADIUS = 0.05  # nodes within this of a transducer are held intact
# geometry
LENGTHS = (1.0, 1.0)
# defects
NUM_VOIDS = 4
...
held = frozen[interior].get().T
shade = np.ma.masked_array(np.full(held.shape, 0.8), ~held)
...
for ax, field, title in zip(
    ...
    ax.imshow(
        shade, origin="lower", extent=[0.0, Lx, 0.0, Ly], cmap="gray", vmin=0, vmax=1
    )
    ax.set_title(title)
    ax.set_aspect("equal")
    ax.set_xticks([])
