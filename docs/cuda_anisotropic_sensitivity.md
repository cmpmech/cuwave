...
### own_offset
**aim**
offset of entry `l` of the cell this node is the low corner of
**input args**
`l`: entry code; `radius`, `S`: as [cuda_anisotropic](cuda_anisotropic.md)
**how?**
$$\textrm{offset}=\sum_d\textrm{bit}_d(l)\,s_d$$

### corner_cell
**aim**
the $2^\textrm{ndim}$ cells a node is a corner of, for the scatter of the chain rule
**input args**
`c`: corner code; `A`, `NN`, `S`: geometry
**output args**
`base`: offset from `idx` to that cell's low corner
