...
## compilation logic
`compile_kernels(sim, KERNEL_PATH)` builds a second `cp.RawModule` from this file with the same `nvcc` flags and the same injected stencil table as the forward one, so radius, precision and dimension always agree between the two modules

The `real_t` typedef and the `OP_W` / `CLOSURE` block are [cuda_wave](cuda_wave.md)'s, repeated verbatim: CuPy compiles each source string on its own, and a shared header would need an include path `RawModule` is never given. `-DUSE_DAMPING` is the one flag that cannot arrive here — `require_lossless` rejects a damping field before either module is compiled, since the adjoint of a damped step is not that step run backwards
...
**aim**
compute one axis of $\partial\left[\lambda\cdot\nabla\cdot(k\nabla u)\right]/\partial k_i$ at $i$ (`idx`), over the two faces the node owns
**input args**
...
**internal args**
`sp, sm`: `stiff` at the two neighbours $k_{i\pm1}$; `dgp, dgm`: the two face stiffnesses differentiated with respect to `sc`; `Dp, Dm`: the two face fluxes $D_i^+, D_i^-$, formed exactly as in `flux_divergence_axis`
**how?**
- $\lambda\cdot\nabla\cdot(k\nabla u)$ is a sum over **faces**, not nodes: the face at $i+\frac{1}{2}$ carries one stiffness $g^+$ and one flux $D_i^+$, and enters node $i$ with a $+$ and node $i+1$ with a $-$
$$\lambda\cdot\nabla\cdot(k\nabla u)=-\sum_d F_d\sum_{\textrm{faces}}g^+D_i^+\left(\lambda_{i+1}-\lambda_i\right)$$
- $k_i$ sits in exactly two of those faces, so the derivative stays local
...
- and the derivative of the (halved) harmonic face mean is as cheap as the mean itself
...
- `Dp` and `Dm` are the **same** face differences the forward kernel forms, off the same loads and the same graded radius — and $D_i^-$ is identically $D_{i-1}^+$, which is what makes the face pairing above hold at every `space_order` instead of only at order 2
- note the asymmetry: $u$ enters through the wide stencil $D^\pm$, $\lambda$ only through a single difference across the face. This is the exact transpose, and it is *not* symmetric, which is why `frechet_kernel` cannot reuse it
...
**how?**
- the mass term is node-local — no neighbour and no material load, just the second time difference of the triplet against $\lambda^t$
- the stiffness term takes the middle slot `u1` alone, because the operator in the residual is evaluated one step back, plus the neighbours of `u1`, `l1` and `stiff`
- both gradients stay separate *outputs*, with separate chain-rule factors at the call site, but one kernel computes them: a launch costs more host time than either body costs on the device, and fusing loads `u1`, `l1` and the interior guard once instead of twice
- both accumulate with `-=` into arrays the driver zeroes once, so a caller may run the kernel over a subset of the steps or with aliased fields
